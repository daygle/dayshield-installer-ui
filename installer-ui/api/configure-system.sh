...
# ── 3. /etc/network/interfaces — loopback only ────────────────────────
# The rootfs uses systemd-networkd for all interface management; installer-
# finalize.sh (called in step 3a) writes the systemd-networkd .network files
# for WAN and LAN.  Keep this file to loopback-only so the ifupdown
# networking.service (if present) does not try to manage the same interfaces.
...
# Run installer-finalize.sh from the HOST (not inside a chroot) so that
# the script receives a valid target path and can itself chroot into the
# target for operations that require it (e.g. chpasswd, init-admin).
# The script writes systemd-networkd .network files for WAN + LAN, the
...
# Note: networking (ifupdown) is intentionally omitted — the rootfs uses
# systemd-networkd for all interface management.
WANTED_SERVICES="ssh wg-quick@wg0"
