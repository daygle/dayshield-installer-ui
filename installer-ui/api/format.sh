...
# Labels match the values the dayshield-rootfs initramfs hook and dayshield-core
# code look up (DS_SYSROOT for root, DAYSHIELD_BOOT for /boot, DS_STATE for /var).
...
err=$(mkfs.ext4 -F -L "DS_SYSROOT" -O "^64bit,metadata_csum" -m 1 "$ROOT_PART" 2>&1) || json_error "Failed to format root partition $ROOT_PART: $err"
